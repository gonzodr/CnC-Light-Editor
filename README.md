...
- Stencil mód: nagy, additívan keveredő fényforrások világítanak az alfa-lyukas playfield artwork mögött. Aktív Canvas/átlátszó háttér esetén a stencil világoskék hátteret mutat a fekete helyett, így ránézésre ellenőrizhető az overlay állapota.
...
- A végleges, 64 KiB fölötti effektbankkal is biztonságos V4 `EffectDef` protokollal kompatibilis Arduino `FX_DATA_PROGMEM` export explicit ID, `frameMs`, `loops`, `loopFrames` és `overlay` metaadatokkal. A fejléc automatikusan generálja az ID → 24 bites far-flash cím leképezést is, ezért új vagy átnevezett effekt után nincs kézzel karbantartandó firmware-switch. Az azonos képkockák is megmaradnak, mert a motor fix frame-idővel játszik.
...
- **XOR-maszkolt effektadat**: az exportált `fx_*` tömbök minden bájtja XOR 0x5A-val TÁROLVA kerül a fejlécbe, és a fejléc egy `#define FX_DATA_MASK_APPLIED 0x5A` sort is kap. Erre azért van szükség, mert a Mega USB-soros útja érzékeny konkrét bájtmintákra: egyetlen 256 bájtos lap nyers effektadat megbízhatóan megölte az avrdude feltöltést a 28%-nál, MINDEN gépen - ugyanaz a lap maszkolva hibátlanul felment. A firmware olvasáskor fejti vissza (`FX_DATA_MASK` a `d_light_effects.ino`-ban), tehát **a két oldalnak ugyanazt az értéket kell használnia**. A betöltés automatikusan visszafejt, ha a fejléc hordozza a define-t; a nélküle érkező (régi) fejlécek változatlanul, nyersként olvasódnak - így egy meglévő `effect_data.h` betöltése és újraexportálása magától átmigrálja a bankot a maszkolt formátumra, kézi lépés nélkül.
- Exportkor egyetlen gördülő biztonsági mentés készül az előző `effect_data.h`-ról (`effect_data.h.bak`); a `Restore backup` gomb azonnal, lemezen is visszaállítja, a lecserélt változatot pedig backupként megtartja, így a művelet újabb kattintással visszafordítható.
- A speciális `F+` Falloff réteg időben simítja a LED-ek kikapcsolását: a fényerő-emelkedés azonnali marad, a csökkenés pedig állítható (100–2000 ms, alapból 600 ms) kifutásként kerül a baked képkockákba. Az enabled állapot a Canvas réteghez hasonlóan keyframe-elhető.
- A bemappelt firmware-bank `Save bank` vagy `Ctrl+S` hatására közvetlenül ugyanabba a fájlba ment, fájlkezelő nélkül, majd azonnal frissíti a bank- és stencilnézetet. A `Save as…` külön megmaradt. Idegen projekt meglévő ID-jának cseréje megerősítést kér, a bank részletei pedig előre jelzik az `ADD`/`REPLACE` műveletet.
...
Windowson, valódi (nem headless) munkamenetben a "Map header…"/"Save as…"/projekt-megnyitás gombok a natív Windows Intéző-ablakot nyitják meg a Pygame-böngésző helyett - a fájlválasztás így az OS-hez szokott billentyűparancsokkal/kedvencekkel megy. A `Save bank` a már bemappelt fejlécet közvetlenül írja. A `Map firmware effect_data.h` útvonala munkamenetek között is megmarad: ha egyszer bemappelsz egy `effect_data.h`-t, a szerkesztő legközelebbi indításkor automatikusan újra betölti ugyanazt a fájlt.
